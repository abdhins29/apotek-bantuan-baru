...
while($dd=mysqli_fetch_array($qq)){
	mysqli_query($konek,"INSERT INTO transaksi_jual VALUES ('','$no_faktur','$dd[kd_obat]','$dd[tgl_jual]','$dd[total_jual]')");
	$id = mysqli_insert_id($konek);
    mysqli_query($konek,"INSERT INTO detail_jual VALUES ('$sub_faktur','$id','$dd[kd_obat]','$dd[jml_jual]')");
	mysqli_query($konek,"DELETE FROM keranjang WHERE idkeranjang='$dd[idkeranjang]'");
}
echo "<script>window.location.href='index.php?page=kelolajual'</script>";
?>